...
import (
	...
	"envoy-wasm-graphql-federation/pkg/planner" // 从正确的包导入
...
func TestPlanner_CreateExecutionPlan_NilParameters(t *testing.T) {
	...
	// 测试 nil 查询
	_, err := plannerInstance.CreateExecutionPlan(ctx, nil, []types.ServiceConfig{})
	if err == nil {
	...
	_, err = plannerInstance.CreateExecutionPlan(ctx, query, []types.ServiceConfig{})
	if err == nil {
